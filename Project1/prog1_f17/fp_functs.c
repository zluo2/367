...
// Also, return -1 if the sum overflows
  ...
  //check which exp is greater
  if (e1 > e2)
  {
    int different = e1 - e2;
    //find the different between E
  ...
  //when two frac have the same E, we can add them
  float newFrac = frac1 + frac2;

  //if the fraction is greater than 2
  //justify fraction down
  //(e1 = e2)
  if (newFrac >= 2)
  {
    newFrac = newFrac / 2;
